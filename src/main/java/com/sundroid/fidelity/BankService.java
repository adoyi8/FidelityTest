...
import com.sundroid.fidelity.user.AppUserRepository;
...
public class BankService {
    ...
    public ResponseEntity<Map<String, Object>> withdraw(AppUser user, WithdrawalModel withdrawalModel) {
        ...
        if(user.isEnabled()) {
            if (withdrawalModel.getAmount() != null && withdrawalModel.getAmount() > 1.0) {
            ...
            response.put("responseCode", HttpStatus.BAD_REQUEST.value());
            response.put("success", false);
            response.put("message", "This Account has been disable, please contact support for more information");
            return ResponseEntity.badRequest().body(response);
        }
    }
    ...
    public ResponseEntity<Map<String, Object>> disableUser(AppUser user, AppUser account) {
        Map<String, Object> response = new HashMap<>();
        if (account.getEmail() == null) {
            ...
            Optional<AppUser>  accountToBeDisabled = appUserRepository.findByEmail(account.getEmail());
            if (!accountToBeDisabled.isPresent()) {
                response.put("responseCode", HttpStatus.BAD_REQUEST.value());
                response.put("success", false);
                response.put("message", "Account does not exist!");
                return ResponseEntity.badRequest().body(response);
            }  else if (!accountToBeDisabled.get().isEnabled()) {
                response.put("responseCode", HttpStatus.BAD_REQUEST.value());
                response.put("success", false);
                response.put("message", "Account already disabled");
                return ResponseEntity.badRequest().body(response);
            } else {
                accountToBeDisabled.get().setEnabled(false);
                appUserRepository.save(accountToBeDisabled.get());
                response.put("responseCode", HttpStatus.OK.value());
                response.put("success", true);
    ...
    public ResponseEntity<Map<String, Object>> enableUser(AppUser user, AppUser account) {
        Map<String, Object> response = new HashMap<>();
        ...
        if (account.getEmail() == null) {
            ...
            Optional<AppUser>  accountToBeEnabled = appUserRepository.findByEmail(account.getEmail());
            if (!accountToBeEnabled.isPresent()) {
                response.put("responseCode", HttpStatus.BAD_REQUEST.value());
                response.put("success", false);
                response.put("message", "Account does not exist!");
                return ResponseEntity.badRequest().body(response);
            }  else if (accountToBeEnabled.get().isEnabled()) {
                response.put("responseCode", HttpStatus.BAD_REQUEST.value());
                response.put("success", false);
                response.put("message", "Account already enabled");
                return ResponseEntity.badRequest().body(response);
            } else {
                accountToBeEnabled.get().setEnabled(true);
                appUserRepository.save(accountToBeEnabled.get());
                response.put("responseCode", HttpStatus.OK.value());
                response.put("success", true);
    ...
    public ResponseEntity<Map<String, Object>> deleteUser(AppUser user, AppUser account) {
        Map<String, Object> response = new HashMap<>();
        ...
        if (account.getEmail() == null) {
            ...
            Optional<AppUser>  accountToBeDeleted = appUserRepository.findByEmail(account.getEmail());



            if (!accountToBeDeleted.isPresent()) {
                response.put("responseCode", HttpStatus.BAD_REQUEST.value());
                response.put("success", false);
                response.put("message", "Account does not exist!");
                return ResponseEntity.badRequest().body(response);
                ...
                accountToBeDeleted.get().setEnabled(false);
                appUserRepository.delete(accountToBeDeleted.get());
                response.put("responseCode", HttpStatus.OK.value());
                response.put("success", true);
    ...
    public ResponseEntity<Map<String, Object>> makeAdmin(AppUser user, AppUser account) {
        Map<String, Object> response = new HashMap<>();
        ...
        if (account == null || account.getEmail() == null) {
            ...
            Optional<AppUser>  accountToBeUpdated = appUserRepository.findByEmail(account.getEmail());
            if (!accountToBeUpdated.isPresent()) {
                response.put("responseCode", HttpStatus.BAD_REQUEST.value());
                response.put("success", false);
                response.put("message", "Account does not exist!");
                return ResponseEntity.badRequest().body(response);
            }  else if (accountToBeUpdated.get().getAppUserRole().equals(AppUserRole.ADMIN)) {
                response.put("responseCode", HttpStatus.BAD_REQUEST.value());
                response.put("success", false);
                response.put("message", "This user is already an admin");
                return ResponseEntity.badRequest().body(response);
            } else {

                accountToBeUpdated.get().setAppUserRole(AppUserRole.ADMIN);
                appUserRepository.save(accountToBeUpdated.get());
                response.put("responseCode", HttpStatus.OK.value());
                response.put("success", true);
    ...
    public ResponseEntity<Map<String, Object>> removeAdmin(AppUser user, AppUser account) {
        Map<String, Object> response = new HashMap<>();
        ...
        if (account == null || account.getEmail() == null) {
            ...
            if (!accountToBeUpdated.isPresent()) {
                ...
                response.put("message", "Account does not exist!");
                return ResponseEntity.badRequest().body(response);
            }  else if (accountToBeUpdated.get().getAppUserRole().equals(AppUserRole.USER)) {
                response.put("responseCode", HttpStatus.BAD_REQUEST.value());
                response.put("success", false);
                response.put("message", "This user is not an admin");
                return ResponseEntity.badRequest().body(response);
            } else {

                accountToBeUpdated.get().setAppUserRole(AppUserRole.USER);
                appUserRepository.save(accountToBeUpdated.get());
                response.put("responseCode", HttpStatus.OK.value());
                response.put("success", true);
                response.put("message", "Operation successful!");
